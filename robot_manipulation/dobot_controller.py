...
class DobotController():
    ...
    def __init__(self, color):
        ...
        self.move_arm(self.home_pos[0],self.home_pos[1],self.home_pos[2],0,wait=True)
        ...
        try:
            ...
            if retry:
                ...
                while retry_cnt < retry_limit:
                    (x_tmp, y_tmp, z_tmp, r_tmp, j1_tmp, j2_tmp, j3_tmp, j4_tmp) = self.device.pose()
                    try:
                        print(f"\nRetrying: {retry_cnt+1}/{retry_limit}")
                        self.device.move_to(x_tmp+1, y_tmp+1, z_tmp+1, r_tmp, wait=True)
    ...
    def perform_move(self, move = [1,1], crown = False, height = 10):
        ...
        self.move_arm(self.board[x][y][0],self.board[x][y][1],self.board[x][y][2]+height,0,wait=True)
        self.move_arm(self.board[x][y][0],self.board[x][y][1],self.board[x][y][2],0,wait=True)
        self.device.suck(True)
        self.move_arm(self.board[x][y][0],self.board[x][y][1],self.board[x][y][2]+height,0,wait=True)
        ...
        for i in range(1,len(move) -1 ,1):
            if move[i] < 0:
                ...
                self.device.suck(True)
                self.move_arm(self.board[x][y][0],self.board[x][y][1],self.board[x][y][2]+height,0,wait=True)
                self.move_arm(self.dispose_area[0],self.dispose_area[1],self.dispose_area[2],0,wait=True)
                self.device.suck(False)
        ...
        if crown == True:
            ...
            self.move_arm(self.side_pockets[xk][yk][0],self.side_pockets[xk][yk][1],self.side_pockets[xk][yk][2]+height,0,wait=True)
            self.move_arm(self.side_pockets[xk][yk][0],self.side_pockets[xk][yk][1],self.side_pockets[xk][yk][2],0,wait=True)
            self.device.suck(True)
            self.move_arm(self.side_pockets[xk][yk][0],self.side_pockets[xk][yk][1],self.side_pockets[xk][yk][2]+height,0,wait=True)
            self.move_arm(self.board[x][y][0],self.board[x][y][1],self.board[x][y][2]+height,0,wait=True)
            self.move_arm(self.board[x][y][0],self.board[x][y][1],self.board[x][y][2],0,wait=True)

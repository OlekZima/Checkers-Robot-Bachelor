...
class CheckersGame:
    ...
    @classmethod
    def _get_value_of_tile(cls, tile_id, game_state):
        x, y = get_coord_from_tile_id(tile_id)

        return game_state[x][y]

    @classmethod
    def _get_man_poss_moves(cls, tile_id, game_state):
        poss_moves = []
        ...
        if val_of_id == 1:  # ORANGE -> moves 'down' towards bigger ids
            x, y = get_coord_from_tile_id(tile_id)  # poss moves to x-1,y+1 or x+1, y+!
    ...
    @classmethod
    def _get_king_poss_moves(cls, tile_id, game_state):
        poss_moves = []
        val_of_id = CheckersGame._get_value_of_tile(tile_id, game_state)

        if val_of_id not in (-2, 2):
            return None  # this tile doesn't contain a king piece of either player
    ...
    @classmethod
    def _get_king_poss_jumps(cls, tile_id, game_state, prev_seq=None):
        if prev_seq is None:
            prev_seq = []
        poss_jumps = []
        val_of_id = CheckersGame._get_value_of_tile(tile_id, game_state)
    ...
    def get_color_poss_opts(cls, color, game_state):
        ...
        for x, _ in enumerate(game_state):  # iterate all tiles for pieces
            for y, _ in enumerate(game_state[x]):
                tile_id = get_tile_id_from_coord(x, y)

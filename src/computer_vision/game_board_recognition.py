...
class Board:
    ...
    def is_00_white(self, radius=4, dark_field_bgr=[0, 0, 0], light_field_bgr=[255, 255, 255], red_bgr=[0, 0, 255],
                    green_bgr=[0, 255, 0], color_dist_thresh=60):
        ...
        if (
                distance_from_color(sample_avg_bgr, dark_field_bgr) < distance_from_color(sample_avg_bgr,
                                                                                          light_field_bgr)
...
class BoardTile:
    ...
    def get_tiles_contours(cls):
        ...
        for t in BoardTile.tiles:
            contours = np.append(contours, [[[t.vertexes[0]], [t.vertexes[1]], [t.vertexes[2]], [t.vertexes[3]]]],
                                 axis=0)
        return contours[1:]
    ...
    def is_point_in_rad_range(self, rad_min, rad_max, point=[0, 0]):
        ...
        if (rad_min <= rad_max and dir_tmp >= rad_min and dir_tmp <= rad_max) or (
                rad_min > rad_max and dir_tmp <= rad_max) or (rad_min > rad_max and dir_tmp >= rad_min):
    ...
    def get_num_of_steps_in_dir_rad(self, dir_rad, dir_idx):  # recursive function for indexing steps
        ...
        if same_dir is not None:
            results.append(same_dir.get_num_of_steps_in_dir_rad(dir_rad,
                                                                dir_idx) + 1)  # one more because it is in checked direction
    ...
    def index_neighbours(self, dir_0):
        if self.x_idx is None or self.y_idx is None:
            ...
            cv.putText(BoardTile.frame, f'{self.x_idx},{self.y_idx}', [self.center[0] - 5, self.center[1]],
...
def main():
    while True:
        ...
        contours = get_game_tiles_contours(img, t1=t1, t2=t2, kernel=np.ones((kernel_size, kernel_size)),

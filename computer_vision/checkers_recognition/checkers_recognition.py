...
class Checkers:
    ...
    @classmethod
    def detect_checkers(cls, board, frame, bgr_red = [50,80,220], bgr_green = [205,110,60], color_dist_thresh = 7):
        ...
        for x in range(0,len(board.points)-1,1):
            for y in range(0,len(board.points[x])-1,1):
                ...
                if detected_checker_color is not None:
                    Checkers.checkers.append(Checkers(pos=[x,y],color=detected_checker_color))
